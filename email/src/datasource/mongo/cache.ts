...
export const createCachingMethods = ({ collection, model, cache }) => {
  const loader = new DataLoader(async ejsonArray => {
    const fieldsArray = ejsonArray.map((item:any)=>EJSON.parse(item))
    log('fieldsArray', fieldsArray)

    const filterArray = fieldsArray.reduce((filterArray:any, fields:any) => {
  ...
  const methods = {
    findOneById: async (_id, { ttl }:any = {}) => {
      ...
      const docs = await loader.load(EJSON.stringify({ _id })) as any
    ...
    findManyByIds: (ids, { ttl }:any = {}) => {
      return Promise.all(ids.map(id => methods.findOneById(id, { ttl })))
    },
    findByFields: async (fields, { ttl }:any = {}) => {
